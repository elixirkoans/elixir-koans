defmodule Structs do
  ...
  koan "Structs are defined and named after a module" do
    person = %Person{}
    assert person == %Person{name: nil, age: nil}
  end

  koan "Structs are also maps equivalent" do
    %{name: n, age: a} = %Person{name: "Bill", age: 25}
    assert n == "Bill"
  end
  ...
  koan "Use the put_in macro to replace a nested value" do
    airline = %Airline{}
    assert put_in(airline.plane.maker, :airbus) == %Airline{name: "Southwest", plane: %Plane{passengers: 0, maker: :airbus}}
  end

  koan "Use the update_in macro to modify a nested value" do
    airline = %Airline{plane: %Plane{passengers: 200}}
    assert update_in(airline.plane.passengers, fn x -> x + 2 end) == %Airline{name: "Southwest", plane: %Plane{passengers: 202, maker: :boeing}}
  end
